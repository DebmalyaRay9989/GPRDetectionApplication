...
with tab_single:
    ...
    with col_left:
        ...
        with st.expander("⚙  SGY Preprocessing Config" + (" — active" if _is_sgy else ""),
                         expanded=_is_sgy):
            pp_col1, pp_col2, pp_col3 = st.columns(3, gap="small")
            ...
            with pp_col1:
                ...
                pp_gain_db = st.slider("Max Gain (dB)", 5.0, 60.0, 30.0, 1.0)
                pp_agc_win = st.slider("AGC Half-window", 5, 80, 20, 5,
                                       help="Only used when Gain Mode = agc.")

            with pp_col2:
                st.markdown("<div class='sec-label'>Filtering</div>", unsafe_allow_html=True)
                pp_dewow_ui    = st.checkbox("Apply Dewow", value=True)
                pp_dew_win     = st.slider("Dewow Window", 3, 50, 39, 1,
                                           disabled=not pp_dewow_ui)
                pp_bandpass_ui = st.checkbox("Apply Bandpass", value=False)
                pp_bp_lo       = st.number_input("BP Low (MHz)",  value=100.0, step=10.0,
                                                 disabled=not pp_bandpass_ui)
                pp_bp_hi       = st.number_input("BP High (MHz)", value=900.0, step=10.0,
                                                 disabled=not pp_bandpass_ui)
                pp_bp_ord      = st.selectbox("Butterworth Order", [2, 4, 6, 8], index=1,
                                              disabled=not pp_bandpass_ui)

            with pp_col3:
                st.markdown("<div class='sec-label'>Background / Output</div>", unsafe_allow_html=True)
                pp_bg_mode    = st.radio("BG Removal", ["mean", "median"])
                pp_trace_norm = st.checkbox("Trace Normalisation (RMS)")
        ...
        if uploaded:
            if _is_sgy:
                ...
                if _pp_result["status"] != "OK":
                    ...
                    st.image(img,
                             caption=f"📡 Preprocessed scan — {img.size[0]}×{img.size[1]} px",
                             width=_disp_w)
                    st.markdown(f"""
